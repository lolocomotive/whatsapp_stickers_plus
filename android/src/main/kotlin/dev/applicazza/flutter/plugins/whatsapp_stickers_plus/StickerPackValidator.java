...
class StickerPackValidator {
    ...
    static void verifyStickerPackValidity(@NonNull Context context, @NonNull StickerPack stickerPack)
        ...
        try {
            final byte[] bytes = StickerPackLoader.fetchStickerAsset(stickerPack.identifier, stickerPack.trayImageFile,
                    context);
    ...
    private static void validateSticker(@NonNull Context context, @NonNull final String identifier,
            @NonNull final Sticker sticker) throws InvalidPackException {
    ...
    private static void validateStickerFile(@NonNull Context context, @NonNull String identifier,
        ...
        try {
             ...
             * throw new InvalidPackException(InvalidPackException.
             * INCORRECT_IMAGE_SIZE,"sticker height should be " + IMAGE_HEIGHT +
             * ", sticker pack identifier:" + identifier + ", filename:" + fileName);
             * }
             * if (webPImage.getWidth() != IMAGE_WIDTH) {
             * throw new InvalidPackException(InvalidPackException.
             * INCORRECT_IMAGE_SIZE,"sticker width should be " + IMAGE_WIDTH +
             * ", sticker pack identifier:" + identifier + ", filename:" + fileName);
             * }
             * // if (webPImage.getFrameCount() > 1) {
             * // throw new InvalidPackException(InvalidPackException.
             * ANIMATED_IMAGES_NOT_SUPPORTED,"sticker should be a static image, no animated sticker support at the moment, sticker pack identifier:"
             * + identifier + ", filename:" + fileName);
             * // }
             * } catch (IllegalArgumentException e) {
             * throw new InvalidPackException(InvalidPackException.
             * UNSUPPORTED_IMAGE_FORMAT,"Error parsing webp image, sticker pack identifier:"
             * + identifier + ", filename:" + fileName);
             * }
             */
        } catch (IOException e) {
